function [U_, CEC] = PensionFundSE(xx, E, afx, nSim, T, gamma, spread)
...
%         end
        ...
        if afx == 3
            ...
            sum_(:,:,1) = expm(-E.K(:,:,s));
            for i = 2:Tw
                sum_(:,:,i) = sum_(:,:,i-1) + expm(-i * E.K(:,:,s));
            end
            w_tilde = zeros(T,Tw);
            W(:,1)  = sum(discB(:,:,1),1)';
            for tau=1:Tw
                w_tilde(1,tau)  = E.w(1,s) * exp(tau * E.delta_0pi(:,s) + E.delta_1pi(:,s)' * sum_(:,:,tau) * E.X(:,s,1));
            end
            for j = 1:Tw
        ...
        for t = 2:T
            ...
            if afx == 3
                W(:,t)  = sum(discB(:,:,t),1)';
                for tau=1:Tw
                    w_tilde(t,tau) = E.w(t,s) * exp(tau * E.delta_0pi(:,s) + E.delta_1pi(:,s)' * sum_(:,:,tau) * E.X(:,s,t));
                end
                for jt=1:Tw
...
%         end
        ...
        if isnan(CEC(s)) || CEC(s) == 0 || sum(isinf(U_(:,s))) > 0 || sum(isnan(U_(:,s))) > 0
           disp('alarm'); 
           bankrupt(s) = 1;
        end
    end
    ...
    U_welfare = U_(:, ~bankrupt);
    welfare = mean(U_welfare, 2);
